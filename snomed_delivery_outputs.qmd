---
title: National SNOMED delivery code frequency
output:
  html_document:
  theme: journal
github_document:
...
## 2. Codelist specific total code prevalence over time 
del_codelist_data_summary_plot <- ggplot(data = codelist_data_summary, aes(x = year, y = total_usage)) +
    geom_line(aes(group=1)) +
    geom_point() +
...
# Select top 10 most frequently used codes by proportional usage
del_top_10_codes_each_year <- del_codelist_data_percent %>% 
  mutate(snomed_code_description = paste(snomed_concept_id, description, sep = " - ")) %>%
  group_by(year) %>%
  slice_max(prop_usage, n = 10) %>%
  ungroup() %>%
  mutate(name_lab = if_else((year == "2023")| (year == "2017" & snomed_code_description == "47340003 - Birth weight finding (finding)"), snomed_code_description, NA_character_))
...
ggsave(here::here(output_dir_rmd,"figures","top_10_snomed_del_codes_all_years_plot.png"), del_top_10_codes_all_years_plot)
