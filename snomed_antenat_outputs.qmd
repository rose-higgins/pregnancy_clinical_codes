---
title: National SNOMED antenatal code frequency
output:
  html_document:
  theme: journal
github_document:
...
<!-- # ###################################################################################################################
# Script creating plots of key outputs for analysing SNOMED antenatal codelist trends over time and codelist changes over time
#
...
# Input directory ----
input_dir_os <- here("snomed_antenat_outputs")

# Output directory ----
output_dir_rmd <- here("snomed_antenat_outputs")
...
```{r, coverage plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "Figure 1 Total national SNOMED antenatal codes frequency by financial year", fig.topcaption=TRUE}
...
```{r, codelist summary trend, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "Figure 2 Total national SNOMED code frequency for the antenatal SNOMED codelist by financial year", fig.topcaption=TRUE}
########################################
# Codelist specific outputs

# Import the codelist
pregnancy_codelist <- get_codelist("user/VickiPalin/pregnancy_antenatal_snomed/61667fb8/")
...
codelist_data_summary_plot <- ggplot(data = codelist_data_summary, aes(x = year, y = total_usage)) +
    geom_line(aes(group=1)) +
    geom_point() +
    labs(title = "Antenatal codelist SNOMED Code Usage by Year",
                x = "Year",
                y = "Total Usage") +
    theme(text = element_text(size = 15)) +
    scale_y_continuous(breaks = scales::pretty_breaks(n = 10), labels = scales::label_number(accuracy = 1)) +
...
ggsave(here::here(output_dir_rmd,"figures","snomed_antenat_codelist_summary_plot.png"), codelist_data_summary_plot)

include_graphics(fs::path(output_dir_rmd,"figures","snomed_antenat_codelist_summary_plot.png"))
```
...
top_10_codes_table %>%
  ...
  save_kable(file = "snomed_antenat_outputs/tables/top_10_snomed_antenat_codes_table.html"
              , zoom = 1.5)
webshot2::webshot("snomed_antenat_outputs/tables/top_10_snomed_antenat_codes_table.html", "snomed_antenat_outputs/tables/top_10_snomed_antenat_codes_table.png")

# include_graphics(fs::path(output_dir_rmd,"tables","top_10_snomed_antenat_codes_table.png"))
```
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "Table 1 Top 10 most frequent SNOMED codes recorded for the antenatal SNOMED codelist in financial year 2023-24", fig.topcaption=TRUE}
include_graphics(fs::path(output_dir_rmd,"tables","top_10_snomed_antenat_codes_table.png"))
```

```{r, codelist top 10 trend over time, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "Figure 3 Top 10 most frequent SNOMED codes recorded for the antenatal SNOMED codelist in financial year 2023-24", fig.topcaption=TRUE}
...
top_10_allyear_codes <- codelist_data_percent %>%
  group_by(year) %>%
  slice_max(prop_usage, n = 10) %>%
  pull(snomed_concept_id) %>%
  unique()

top_10_allyear_codes <- data.frame(snomed_concept_id = c(top_10_allyear_codes))

# Filter data to only those snomed codes which were in the top 10 in any year 
top_codes_data <- codelist_data_percent %>%
  filter(snomed_concept_id %in% top_10_allyear_codes$snomed_concept_id)
...
top_10_codes_each_year <- top_codes_data %>% 
  ...
  mutate(name_lab = if_else((year == "2023")| (year == "2020" & snomed_code_description == "77386006 - Pregnant (finding)"), snomed_code_description, NA_character_))

# write results to a table
write.csv(top_10_codes_each_year, file ="snomed_antenat_outputs/tables/top_10_snomed_antenat_codes_each_year.csv")    
...
ggsave(here::here(output_dir_rmd,"figures","top_10_snomed_antenat_codes_all_years_plot.png"), top_10_codes_all_years_plot)

include_graphics(fs::path(output_dir_rmd,"figures","top_10_snomed_antenat_codes_all_years_plot.png"))
```