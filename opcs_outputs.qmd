---
title: National OPCS-4 code frequency
output:
  html_document:
  theme: journal
github_document:
...
# National OPCS-4 code frequency trends over time
...
opcs_summary_plot <- ggplot(data = opcs_summary, aes(x = year, y = total_usage)) +
    ...
    geom_point() +
    labs(title = "Total OPCS-4 Code Usage by Year",
                x = "Year",
                y = "Total Usage") +
...
codelist_data_summary_plot <- ggplot(data = codelist_data_summary, aes(x = year, y = total_usage)) +
    geom_line(aes(group=1)) +
    geom_point() +
    labs(title = "Codelist specific OPCS-4 Code Usage by Year",
                x = "Year",
                y = "Total Usage") +
    theme(text = element_text(size = 15)) +
    scale_y_continuous(breaks = scales::pretty_breaks(n = 10), labels = scales::label_number(accuracy = 1))
...
```{r, codelist top 10 trend over time, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "Figure 3 Top 10 most frequent OPCS-4 codes recorded for the pregnancy OPCS-4 codelist in financial year 2023-24", fig.topcaption=TRUE}
...
top_10_codes_all_years_plot <- ggplot(data = top_10_codes_each_year, aes(x = year, y = usage)) +
    ...
    ggrepel::geom_text_repel(aes(x = year,
                                 ...
                                 label = stringr::str_wrap(opcs_code_description, 35),
                                 fontface = 'bold'),
                                 lineheight = 0.7,
                                 hjust = 1,
                                ...
                                #  segment.color = 'transparent',
                                 ...
                                 box.padding = .2,
                                 force = .8) + 
    # geom_richtext(data = graph_label, aes(label = opcs_code_description), colour = "black",
    ...
    # geom_label() +
    # geom_text_repel() + 
    labs(title = "Top 10 OPCS Codes Usage by Year",
       x = "Year",
       y = "Usage",
       color = "OPCS Code") +
